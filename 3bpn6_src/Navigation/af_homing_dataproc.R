...
# Script last edited 2022-04-26

# for Pašukonis et al. 2022: sex differences in poison frog spatial behavior
# Homing data processing and export for A. femoralis
...
trajectory.df <- read.csv(file="af_homing_traj.csv", header = T, sep =",")
territory.df <- read.csv(file="af_territorypts.csv", header = T, sep =",")
...
#af20_18 not in the dataset because lost during track and found home. Should be included just for homing success
#af17_4 & af17_30 should be exlucded for SC because too few points
#af20_2 and af20_20 trajectories should be manually truncated for duration because stopped very close to home area edge
trajectory.df <- trajectory.df%>%
  filter(!(id == "af20_2" & dt > "2020-01-26 18:50:00"))
trajectory.df <- trajectory.df%>%
  filter(!(id == "af20_20" & dt > "2020-02-14+07:36:00"))
...
nrow(trajectory.df) + nrow(territory.df)#all
nrow(filter(trajectory.df, dt < "2018-01-01")) +
  nrow(filter(territory.df, dt < "2018-01-01")) #2017
nrow(filter(trajectory.df, dt > "2018-01-01")) +
  nrow(filter(territory.df, dt > "2018-01-01")) #2020
...
morning_pts <- evening_pts%>%
  mutate(date = date + 1, time = "06:30:00", dt = paste(date, time))%>%
  dplyr::select(id, dt, x_utm, y_utm, sex)
...
tracks_pts <- trajectory.df %>%
  ...
  st_as_sf(coords = c("x_utm", "y_utm"), crs = 32622)
...
plot(tracks_lines["id"], axes = TRUE)
...
#Import weather data
rain2017 <- read.csv(file="parare2017_rainfall_summaries.csv", header = T, sep =",")
rain2020 <- read.csv(file="parare2020_rainfall_summaries.csv", header = T, sep =",")
temp2017 <- read.csv(file="parare2017_daytime_temp.csv", header = T, sep =",")
temp2020 <- read.csv(file="parare2020_daytime_temp.csv", header = T, sep =",")

rain <- bind_rows(rain2017, rain2020)
temp <- bind_rows(temp2017, temp2020)
...
#export angular data
write.csv(orient, file="af_angular.csv")
...
ggplot() +
  ...
  geom_circle(aes(x0 = 0, y0 = 0, r = 250), linetype = "solid", size = 2, color = "#E7B800") +
...
ggplot() +
  ...
  geom_circle(aes(x0 = 0, y0 = 0, r = 60), linetype = "solid", size = 2, color = "#E7B800") +
...
#add af20_18 to the dataset manually for barplots lost during tracking but found back home
af20_18 <- data.frame(id = "af20_18", sex = "male", trans_group = "200m", homing_bin = 1)
homing_variables <- bind_rows(homing_variables, af20_18)
...
trajectory_export <- trajectory%>%
  ...
  dplyr::select(id, sex, trans_group, trans_dist, homing_prop, homing_bin,
                ...
                duration, time_norm)
write.csv(trajectory_export, file="af_homing_traj_export.csv")
...
#af17_4 and af17_30 have too few pts for SC. Replace respective cells with NA
homing_export[c(18,23),11] <- NA
write.csv(homing_export, file="af_homing_export.csv")
