...
# Homing data processing and export for D. tinctorius
...
trajectory.df <- read.csv(file="dt_homing_traj.csv", header = T, sep =",")
territory.df <- read.csv(file="dt_territorypts.csv", header = T, sep =",")
...
#dt17_1, dt17_6, dt17_7, dt17_15, dt17_19, dt17_26 
#dt19_7, dt19_14, dt19_17, dt19_45, dt19_46 have too few pts for SC. 
#Exluded in the final output but included for homing success

#Remove "dt17_2" because lost tag
trajectory.df <- filter(trajectory.df, id != "dt17_2")
territory.df <- filter(territory.df, id != "dt17_2")
...
nrow(filter(trajectory.df, dt > "2018-01-01")) +
  nrow(filter(territory.df, dt > "2018-01-01")) #2019
...
rain2019 <- read.csv(file="parare2019_rainfall_summaries.csv", header = T, sep =",")
temp2017 <- read.csv(file="parare2017_daytime_temp.csv", header = T, sep =",")
temp2019 <- read.csv(file="parare2019_daytime_temp.csv", header = T, sep =",")

rain <- bind_rows(rain2017, rain2019)
temp <- bind_rows(temp2017, temp2019)
...
#export angular data
write.csv(orient, file="dt_angular.csv")
...
ggplot() +
  ...
  geom_circle(aes(x0 = 0, y0 = 0, r = 250), linetype = "solid", size = 2, color = "#0072B2") +
...
ggplot() +
  ...
  geom_circle(aes(x0 = 0, y0 = 0, r = 60), linetype = "solid", size = 2, color = "#0072B2") +
...
trajectory_export <- trajectory%>%
  ...
  dplyr::select(id, sex, trans_group, trans_dist, homing_prop, homing_bin,
                ...
                duration, time_norm)
write.csv(trajectory_export, file="dt_homing_traj_export.csv")
...
homing_export[c(1,7,10,16,26,27,34,37,61,62,65),11] <- NA

write.csv(homing_export, file="dt_homing_export.csv")
